...
/* Color coding for sleep sliders */
.daily-functioning-slider .slider-track { background: #28a745; }
.long-term-health-slider .slider-track { background: #dc3545; }
.comfort-experience-slider .slider-track { background: #007bff; }
...
<div class="sleep-test-container">
    ...
    <div class="top-section">
        <div class="header-section">
            <h1>Sleep Personalised</h1>
            <div class="header-description">
                <p>Adjust the sliders below to reflect what matters most to you in sleep, and we'll recommend the best interventions for your priorities.</p>
            </div>
        </div>

        <div class="controls-section">          
            <div class="sliders-container">
                <div class="value-slider daily-functioning-slider">
                    <div class="slider-header">
                        <label class="slider-label">Daily Functioning</label>
                        <span class="slider-percentage" id="dailyFunctioningPercent">50%</span>
                    </div>
                    <input type="range" min="0" max="100" value="50" class="slider-track" id="dailyFunctioningSlider">
                    <p class="slider-description">Sleep that enhances your immediate cognitive and physical performance.</p>
                </div>
                
                <div class="value-slider long-term-health-slider">
                    <div class="slider-header">
                        <label class="slider-label">Long-term Health</label>
                        <span class="slider-percentage" id="longTermHealthPercent">30%</span>
                    </div>
                    <input type="range" min="0" max="100" value="30" class="slider-track" id="longTermHealthSlider">
                    <p class="slider-description">Sleep that supports disease prevention, longevity, and brain health over time.</p>
                </div>
                
                <div class="value-slider comfort-experience-slider">
                    <div class="slider-header">
                        <label class="slider-label">Comfort & Experience</label>
                        <span class="slider-percentage" id="comfortExperiencePercent">20%</span>
                    </div>
                    <input type="range" min="0" max="100" value="20" class="slider-track" id="comfortExperienceSlider">
                    <p class="slider-description">The subjective quality and enjoyment of your sleep experience.</p>
                </div>
            </div>
    ...
    <div class="bottom-section">
        <div class="recommendations-section">
            <div class="recommendations-header">
                ...
                <p>Based on your priorities, here are the sleep interventions that will give you the best results:</p>
                <div class="methodology-note">
                    <p><strong>How scoring works:</strong> Each intervention is scored using a logarithmic scale where each point represents roughly 2× the impact. Expected benefit scores account for realistic success rates, then these are combined using your personal weightings to calculate Weighted Benefit Scores (WBS). Time and Money EROI show efficiency per hour and per dollar respectively. <a href="{{ site.baseurl }}/sleep/value-scoring-framework">Learn more about the methodology.</a></p>
                </div>
            </div>
...
{% for intervention_file in site.data.interventions %}
    ...
    {% if intervention_data.applicable_domains contains "sleep" %}
    "{{ intervention_key }}": {
        name: {{ intervention_data.name | jsonify }},
        description: {{ intervention_data.description | jsonify }},
        values: {
            daily_functioning: {% if intervention_data.values["sleep.daily_functioning"] %}{{ intervention_data.values["sleep.daily_functioning"].pbs | plus: 0.0 }} + Math.log2({{ intervention_data.values["sleep.daily_functioning"].isr | plus: 0.0 }}/100) + Math.log2({{ intervention_data.values["sleep.daily_functioning"].uar | plus: 0.0 }}/100){% else %}0{% endif %},
            long_term_health: {% if intervention_data.values["sleep.long_term_health"] %}{{ intervention_data.values["sleep.long_term_health"].pbs | plus: 0.0 }} + Math.log2({{ intervention_data.values["sleep.long_term_health"].isr | plus: 0.0 }}/100) + Math.log2({{ intervention_data.values["sleep.long_term_health"].uar | plus: 0.0 }}/100){% else %}0{% endif %},
            comfort_experience: {% if intervention_data.values["sleep.comfort_experience"] %}{{ intervention_data.values["sleep.comfort_experience"].pbs | plus: 0.0 }} + Math.log2({{ intervention_data.values["sleep.comfort_experience"].isr | plus: 0.0 }}/100) + Math.log2({{ intervention_data.values["sleep.comfort_experience"].uar | plus: 0.0 }}/100){% else %}0{% endif %}
        },
        resources: {
...
console.log('Loaded sleep interventions:', sleepInterventions);

// Color scheme for sleep values
const colors = {
    daily_functioning: '#28a745',   // Green
    long_term_health: '#dc3545',    // Red
    comfort_experience: '#007bff'   // Blue
};
...
// Get DOM elements
const sliders = {
    daily_functioning: document.getElementById('dailyFunctioningSlider'),
    long_term_health: document.getElementById('longTermHealthSlider'),
    comfort_experience: document.getElementById('comfortExperienceSlider')
};

const percentLabels = {
    daily_functioning: document.getElementById('dailyFunctioningPercent'),
    long_term_health: document.getElementById('longTermHealthPercent'),
    comfort_experience: document.getElementById('comfortExperiencePercent')
};
...
function updateRecommendations() {
    ...
    const scoredInterventions = Object.keys(sleepInterventions).map(key => {
        const intervention = sleepInterventions[key];
